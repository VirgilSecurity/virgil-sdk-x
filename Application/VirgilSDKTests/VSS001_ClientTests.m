...
- (void)setUp {
    ...
    XCTestExpectation * __weak ex = [self expectationWithDescription:@"Testcase should be set up properly."];
    ...
    NSUInteger numberOfRequests = 3 + 1;
...
- (void)test001_createCardAndConfirmIdentity {
    ...
    [self createConfirmedCardWithConfirmationHandler:^(NSError *error) {
        ...
        XCTAssertTrue([self.card isConfirmed], @"Virgil Card should be created confirmed.");
...
- (void)confirmIdentity:(NSDictionary *)identity actionId:(NSString *)actionId withHandler:(void(^)(NSError *error))handler {
    ...
    [self.mailinator getInbox:inbox completionHandler:^(NSArray *metadataList, NSError *error) {
        ...
        [self.mailinator getEmail:metadata.mid completionHandler:^(MEmail *email, NSError *error) {
            ...
            // Extract the code
            NSString *code = [match substringFromIndex:match.length - 6];
            [self.client confirmIdentityWithActionId:actionId code:code ttl:nil ctl:nil completionHandler:^(VSSIdentityType type, NSString *value, NSString *validationToken, NSError *error) {
                if (error != nil) {
                    if (handler != nil) {
                        handler(error);
                    }
